...
$(window).ready(function() {
	...
	$('.next2-section').on('click', function(e){
        e.preventDefault();
        var scrollToElement = $(this).parent().next();
        if($(this).parent().parent().parent().parent().is('header')) scrollToElement = $(this).parent().parent().parent().parent().next();
        $('html, body').animate({
            scrollTop: scrollToElement.offset().top + 4100
        }, 1000);
    });
    ...
    if($('.bg-slider').length && $.fn.backstretch){
        ...
        if($('.bg-slider').attr('data-images')) {
            ...
            $('.bg-slider').backstretch([
                'https://purplecow.plura.io/web/index/images/bg/bg-2024-1.jpg'],
            {
                fade: 1000,
                duration: 8000
            });
    ...
    backgroundImage.each(function(){
        if ($(this).attr("data-img")){
            if($(this).parent().hasClass('demo-page')){
                $(this).css("background-image", "url(https://purplecow.plura.io/web/index/images/" + $(this).data("img") + ")");
            }else{
                $(this).css("background-image", "url(https://purplecow.plura.io/web/index/images/" + $(this).data("img") + ")");
            }
